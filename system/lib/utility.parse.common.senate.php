...
class SenateCommonParseUtility extends RawparseUtility {
  ...
  function ru_div_open(& $parser, & $attrs, $tagname ) {/*{{{*/
    ...
    if ($this->debug_tags) $this->recursive_dump($attrs,0,'FORCE');
    $this->push_container_def($tagname, $attrs);
    return TRUE;
  }/*}}}*/
    ...
    if ( is_array($this->current_tag) && array_key_exists('attrs', $this->current_tag) ) {
      ...
      if ( array_key_exists($this->current_tag['attrs']['ID'],array_flip(array(
		...
		if (is_array($this->current_tag) && !$skip ) {
			...
			// $this->syslog( __FUNCTION__, 'FORCE', $this->get_stacktags() . "--- {$this->current_tag['tag']} " . join(' ', $this->current_tag['attrs']) . '-' . join(' ', $this->current_tag['cdata']) );
