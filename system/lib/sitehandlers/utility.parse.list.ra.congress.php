...
class CongressRaListParseUtility extends RawparseUtility {
  ...
  function ru_span_close(& $parser, $tag) {/*{{{*/
    ...
    $span_class = array_element($this->current_tag['attrs'],'CLASS');
		$span_text  = trim(join(' ', is_array($this->current_tag['cdata']) ? array_filter($this->current_tag['cdata']) : array()));
    $span_type  = array(strtolower(0 < strlen($span_class) ? $span_class : 'desc') => $span_text);
		// if ( $span_class == 'bill-head' ) $this->syslog(__FUNCTION__,__LINE__,"(marker) -- {$span_text} ");
